...
// For now, let's fake it till we make it
size_t generate_cfunc_call(HC_FILE fptr, node_expr* expr, func_t* func, reg_t* struct_ptr){ return 0; }

const char* target_architecture = "x64 Windows";
...
const char* target_linker = "link (MSVC Linker)";
...
int link(const char* output_file, char* link_files){
    ...
    for(char* ptr = link_files; *ptr; ptr++)
        if(*ptr == ',') *ptr = ' ';
    snprintf(buffer, 4095, "link %s %s.o /OUT:%s", link_files, output_file, output_file);
    return system(buffer);
}
